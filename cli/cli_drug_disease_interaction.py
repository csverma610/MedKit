#!/usr/bin/env python
"""
CLI for drug-disease interaction checking.

Usage:
    python cli/cli_drug_disease_interaction.py <drug_name> <disease_name>
    python cli/cli_drug_disease_interaction.py <drug_name> <disease_name> --output output.json
    python cli/cli_drug_disease_interaction.py <drug_name> <disease_name> --verbose
...
def main():
    """Main CLI entry point for drug-disease interaction checking."""
    parser = argparse.ArgumentParser(
        description='Check interactions between medications and medical conditions'
    )
    parser.add_argument(
        'drug',
        nargs='?',
    ...
    try:
        # Get drug-disease interaction information
        result = get_drug_disease_interaction(args.drug, args.disease)
        
        if result:
            print(f"\n{'=' * 70}")
            print(f"Drug-Disease Interaction Analysis")
            print(f"{'=' * 70}")
            
            print(f"\nDrug: {args.drug.title()}")
            print(f"Condition: {args.disease.title()}")
            ...
            if hasattr(result, 'disease_name'):
            ...
            if hasattr(result, 'contraindication_status'):
                print(f"\n--- CONTRAINDICATION STATUS ---")
                print(f"{result.contraindication_status}")
            ...
            print(f"No interaction data found for: {args.drug} and {args.disease}")
